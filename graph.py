...
class Graph:
    ...
    def add_edge(self, origin, destination, distance):
        ...
        # If not, it initializes an empty dictionary for that origin
        # This ensures that each origin airport has its own dictionary of destination airports
        if origin not in self.routes:
            # If the origin airport is not in the routes dictionary, it initializes an empty dictionary for that origin
            self.routes[origin] = {}
    ...
    # I created the load_from_csv method to load flight routes from a CSV file I created
    ...
    def load_from_csv(self, filename):
        ...
        try:
            # This line uses the open() function to open the CSV file in read mode; I'm passing filename which is the name of the CSV file, and newline='' to handle newlines in the file correctly
            with open(filename, newline='') as csvfile:
                ...
                for row in reader:
                    # If the length of the row is not equal to 3..
                    if len(row) != 3:
                        # I print a warning message indicating that the row is invalid and skip it so as not to cause an error
                    ...
                    # I created the origin variable and set it equal to the origin airport code; origin.strip() removes any leading or trailing whitespace from the airport code to ensure that it is clean (meaning no extra spaces)and use the upper() method to convert it to uppercase
                    origin = origin.strip().upper()
                    # I created the destination variable and set it equal to the destination airport code; destination.strip() removes any leading or trailing whitespace from the airport code to ensure that it is clean (meaning no extra spaces)and use the upper() method to convert it to uppercase
                    ...
                    # This is done to ensure that the graph is undirected
    ...
    def shortest_path(self, origin, destination):
        ...
        while queue:
            ...
            if current_node == destination:
                # if it has been processedd, I break out of the loop because this means the shortest path has been found
                break
